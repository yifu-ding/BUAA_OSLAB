...
void init_disk() {
    ...
    for(i = 0; i < nbitblock; ++i) {
        memset(disk[2+i].data, 0xff, BIT2BLK/8);
    }
    // 如果 bitmap 还有剩余，我们不能将最后一块位图块靠后的一部分内容标记为空闲，
    // 因为这些位所对应的磁盘块并不存在，不可被使用。因此，在 fs/fsformat.c 中的 
    // init_disk 函数中，在将所有的位图块都置为 0x1 之后，还需要根据实际情况，将 
    // 多出来的位图标记为 0x0。
