...
// PDX(va) 提取va的页目录索引
#define PDX(va)		((((u_long)(va))>>22) & 0x03FF) // PDX宏实际上就是提取va的页目录项索引,所以要右移22位，再和低12位为1的0x03FF&一下,即可得到页目录索引
// PTX(va) 提取页表项的索引
#define PTX(va)		((((u_long)(va))>>12) & 0x03FF) // PTX宏实际上是用来提取页表项的索引，向右移动12位后低10位则是页表项的索引。
// 将每一页的基址pte的值后12位清零
#define PTE_ADDR(pte)	((u_long)(pte)&~0xFFF // 用来将pte的值后12位清零，因为每一页的基址都应该是后12位为零的，不论是页表，页目录，或者是普通的页。

// page number field of address // Question
...
/* 其对于不同的进程而言，虚拟地址ULIM以上的地方，映射关系都是一样的！
*  这是因为这2G虚拟地址与物理地址的对应，不是由进程管理的，是由内核管理的。
 o     4G ----------->  +----------------------------+------------0x100000000
 o                      |       ...                  |  kseg3
 o                      +----------------------------+------------0xe000 0000
 o                      |       ...                  |  kseg2
...
/*实际上这里的ULIM的值就是0x80000000，即内核与用户区的分界线。 */
