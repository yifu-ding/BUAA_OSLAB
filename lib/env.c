 ...
 */
// 初始化env_free_list，然后按逆序将envs[i]插入。
void env_init(void) {
	int i;
...
env_setup_vm(struct Env* e) {
	...
	// e->env_cr3 = page2pa(p); // also right

	e->env_pgdir = pgdir;	  		/* 这个变量保存了该进程页目录的虚拟地址。*/
 ...
 */
// map each segment at correct virtual address.
static int load_icode_mapper(u_long va, u_int32_t sgsize,
	...
	u_long offset = va - ROUNDDOWN(va, BY2PG);				// 向下取整去除offset
	...
	u_long tempVa = ROUND(va,BY2PG);						// 向上取整
...
int load_elf(u_char *binary, 				// binary 为整个待加载的ELF 文件。
			int size,  						// size 为该ELF 文件的大小。
			u_long *entry_point,  			// entry_point 是一个u_long 变量的地址(相当于引用)，解析出的入口地址会被存入到该位置
			void *user_data,
											// 解析出每个segment 的四个信息
			int (*map)(	u_long va, 			// 该段需要被加载到的虚地址
						u_int32_t sgsize, 	// 该段在内存中的大小
						u_char *bin, 		// 该段在ELF 文件中的内容
						u_int32_t bin_size, // 该段在文件中的大小
						void *user_data))*/ // 函数指针，用于将我们的自定义函数传入进去
